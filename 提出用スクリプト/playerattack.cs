...
public class playerattack : MonoBehaviour
{
    ...
    [SerializeField, Header("必殺技の判定終了フレーム")] private int _EXAttack_FinishCollision;   //必殺技の当たり判定終了フレーム
                                                                                      //必殺技
    public bool _stareAttackF = false;                       //掴み合いのフラグ
    public bool _stare_CollisionF = false;          //掴み合いの当たり判定出現中のフラグ
    private int _stare_Count = 0;                      //掴み合いのカウント
    [SerializeField, Header("掴み合いの判定開始フレーム")] private int _stare_StartCollision;    //掴み合いの当たり判定開始フレーム
    [SerializeField, Header("掴み合いの判定終了フレーム")] private int _stare_FinishCollision;   //掴み合いの当たり判定終了フレーム
    ...
    [SerializeField, Header("掴みの当たり判定")] private GameObject _StareColl;
    ...
    void Update()
    {
        ...
        _Animator.SetBool("_StareAttack", _stareAttackF);             // 
        ...
        //掴みの当たり判定管理
        if (_stare_CollisionF)
        {
            _StareColl.GetComponent<BoxCollider>().enabled = true;
            //Debug.Log("掴み判定が出た");
        }
        else if (_stare_CollisionF == false)
        {
            _StareColl.GetComponent<BoxCollider>().enabled = false;
            _GuardColl.GetComponent<BoxCollider>().enabled = false;
        }
    ...
    //掴み合いの当たり判定を出現させるフレーム管理
    private void _Stare_Collision_Start()
    {
        if (_stare_StartCollision == _stare_Count)
        {
            _stare_CollisionF = true;
        }
        if (_stare_FinishCollision == _stare_Count)
        {
            _stare_CollisionF = false;
        }
    }
    ...
    public void pressRB(InputAction.CallbackContext context)
    {
        // ボタン入力がされた際に、ビットの値( 16 )を更新する
        {
            if (context.started)
            {
                ...
                _playercontrol._setFreemove(true);
            }
            if (context.canceled)
            {
                // ビットを元に戻す
                _setButtonBit(~(uint)Button.rb);
                _playercontrol._setFreemove(false);
            }
        }
    }
    ...
    {
        // つかみ合いイベント中ではない時
        if (_stareControl.getStare() == false)
        {
            // 同時押しの判定
            // 押し始めたとき
    ...
    {
        //Debug.Log( gameObject.name + " >> playerattack >> _consecutiveButtonCount：" + _consecutiveButtonCount);
